...
class _PlantDetailScreenState extends State<PlantDetailScreen> {
  ...
  Future<void> _loadPlant() async {
    try {
      ...
      } else if (widget.plantData != null) {
        _plant = Plant.fromMap(widget.plantId, widget.plantData!);
      }

      if (_plant != null) {
        // Load user's unlock status
  ...
  Widget build(BuildContext context) {
    ...
    final care = plant.localizedCare(false);
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        gradient: LinearGradient(
                          ...
                          colors: [
                            Colors.transparent,
                            UrPlantTheme.primary.withValues(alpha: 0.7),
                          ],
                        ),
                      ),
                    ),
            ...
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                    ...
                    children: [
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            Text(
                              plant.localizedName(false),
                              style: const TextStyle(fontSize: 26, fontWeight: FontWeight.w700,
                                  color: UrPlantTheme.textPrimary, letterSpacing: -0.5),
                            ),
                            ...
                            if (plant.nameKh.isNotEmpty)
                              Padding(
                                padding: const EdgeInsets.only(top: 4),
                                child: Text(plant.nameKh,
                                    style: const TextStyle(fontSize: 16, color: UrPlantTheme.textSecondary)),
                              ),
                          ],
                        ),
                      ),
                  ...
                  // Recent discoverers (AvatarStack pattern)
                  if (_recentDiscoverers.isNotEmpty) ...[
                    _recentDiscoverersSection(),
                    const SizedBox(height: 20),
                  ],

                  if (!isUnlocked) _lockedCard()
                  else ...[
                    if (plant.localizedDescription(false).isNotEmpty) ...[
                      _modernSection('🌿 Plant Details'),
                      const SizedBox(height: 8),
                      Text(plant.localizedDescription(false),
                          style: const TextStyle(fontSize: 14, height: 1.7, color: UrPlantTheme.textSecondary)),
                      const SizedBox(height: 24),
                    ],
                    if (plant.localizedOrigin(false).isNotEmpty) ...[
                      _modernSection('🌍 Origin'),
                      const SizedBox(height: 8),
                      Text(plant.localizedOrigin(false),
                          style: const TextStyle(fontSize: 14, color: UrPlantTheme.textSecondary)),
                      const SizedBox(height: 24),
                    ],
                    if (care != null && care.isNotEmpty) ...[
                      _modernSection('💧 Care Guide'),
                      const SizedBox(height: 12),
                      ..._careTiles(care),
                      const SizedBox(height: 24),
                    ],
                    if (plant.localizedFunFacts(false).isNotEmpty) ...[
                      _modernSection('💡 Fun Facts'),
                      const SizedBox(height: 8),
                      ...plant.localizedFunFacts(false).map((f) => Padding(
                            ...
                            child: Row(
                              ...
                              children: [
                                ...
                                Expanded(child: Text(f, style: const TextStyle(fontSize: 14,
                                    color: UrPlantTheme.textSecondary, height: 1.5))),
                              ],
                            ),
                          )),
                      const SizedBox(height: 24),
                    ...
                    if (_userPlant != null) ...[
                      ...
                      Container(
                        ...
                        child: Row(
                          children: [
                            _miniStat('Discovered', _formatDate(_userPlant!.unlockedAt)),
                            Container(width: 1, height: 30, color: UrPlantTheme.divider),
                            _miniStat('Sightings', '${_userPlant!.sightingCount}'),
                          ],
                        ),
                      ),
                      const SizedBox(height: 24),
                    ...
                    Container(
                      ...
                      child: Column(
                        children: [
                          if (plant.family.isNotEmpty) _taxRow('Family', plant.family),
                          if (plant.genus.isNotEmpty) _taxRow('Genus', plant.genus),
                          if (plant.species.isNotEmpty) _taxRow('Species', plant.species),
                        ],
                      ),
                    ),
                  ],
  ...
  Widget _recentDiscoverersSection() {
    return Container(
      padding: const EdgeInsets.all(14),
      decoration: BoxDecoration(
        color: UrPlantTheme.surfaceCard,
      ...
      child: Row(
        children: [
          // Avatar stack
          SizedBox(
            width: (_recentDiscoverers.length.clamp(0, 4) * 24 + 4).toDouble(),
            height: 36,
            child: Stack(
              children: List.generate(
                _recentDiscoverers.length.clamp(0, 4),
                (i) => Positioned(
                  left: i * 18.0,
                  child: Container(
                    width: 32,
                    ...
                    decoration: BoxDecoration(
                      ...
                      border: Border.all(color: UrPlantTheme.surface, width: 2),
                      color: UrPlantTheme.primaryAccent.withValues(alpha: 0.2),
                    ),
                    child: Center(
                      child: Icon(Icons.person, size: 16,
                          color: UrPlantTheme.primaryMedium.withValues(alpha: 0.6)),
                    ),
                  ),
                ),
              ),
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Text(
                  '${_recentDiscoverers.length} people discovered this',
                ...
                Text(
                  ...
                  style: const TextStyle(fontSize: 11, color: UrPlantTheme.textTertiary),
                ),
              ],
            ),
          ),
  ...
  Widget _modernSection(String text) {
    return Text(text, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w700,
        color: UrPlantTheme.textPrimary, letterSpacing: -0.3));
  }
  ...
  List<Widget> _careTiles(Map<String, dynamic> care) {
    ...
    return keys.where((k) => care[k] != null && care[k].toString().isNotEmpty).map((k) {
      return Padding(
        padding: const EdgeInsets.only(bottom: 8),
        child: Row(
          children: [
        ...
        children: [
          SizedBox(
            ...
            child: Text(label, style: const TextStyle(fontSize: 13,
                color: UrPlantTheme.textTertiary, fontWeight: FontWeight.w500)),
          ),
          Text(value, style: const TextStyle(fontSize: 14,
              color: UrPlantTheme.textPrimary, fontWeight: FontWeight.w500)),
        ],
      ),
    );
  }
      ...
      child: Column(
        children: [
          Container(
            width: 64, height: 64,
            decoration: BoxDecoration(
              color: UrPlantTheme.textTertiary.withValues(alpha: 0.1),
              shape: BoxShape.circle,
            ),
          ...
          const Text(
            ...
            style: TextStyle(fontSize: 15, color: UrPlantTheme.textTertiary, height: 1.4),
          ),
        ],
      ),
    );
